...
export class CmpHttpComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.tareasService.getTareas().subscribe(datos => {
      console.log(datos);
    });
  }
  ...
  crearTarea() {
    ...
    this.tareasService.guardarTarea(tarea).subscribe((tareaCreada) => {
      console.log(tareaCreada);
    });
  }

  completarTarea() {
    this.tareasService.completarTarea(7, false).subscribe((tareaCompletada) => {
